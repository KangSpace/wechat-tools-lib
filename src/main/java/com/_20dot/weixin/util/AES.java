...
public class AES {
    ...
    private Cipher getAESCipher(int cipherMode) throws Exception {
        ...
        //此处不能使用new SecureRandom(),使用会出现Linux下生成key不同,Windows下正常
        SecureRandom random = new SecureRandom().getInstance("SHA1PRNG");
        random.setSeed(secretByte);
        ...
        if(!encrypt_aes.equalsIgnoreCase(AES)) {
            ...
            cipher.init(cipherMode, new SecretKeySpec(kgen.generateKey().getEncoded(),encrypt_aes), iv);
        }else {
